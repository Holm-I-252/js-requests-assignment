...
function sendQuery() {
    axios.get('http://localhost:3000/query-test/?name=Ian ?name=bill')
        .then((res) => {
            console.log(res.data)
        })
}
...
// 1. It console logs that I sent an empty query.

// 2. I sent a querry with 2 names, so it console logged name, then a value of the name and the other query attached to it.
...
function createFood(event) {
    ...
    axios.post('http://localhost:3000/food', foods)
        .then((res) => {
            console.log(res.data)
        })
    document.body.append(foods[foods.length - 1])
}

document.getElementById('submit').addEventListener('click', createFood)
